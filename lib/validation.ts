...
export const updateProfileSchema = z.object({
  ...
  heightCm: z.number().min(120).max(250).refine((val) => !isNaN(val), { message: "Height must be a valid number" }).optional(),
  weightKg: z.number().min(30).max(300).refine((val) => !isNaN(val), { message: "Weight must be a valid number" }).optional(),
  ...
  scheduleDaysPerWeek: z.number().int().min(3).max(6).refine((val) => !isNaN(val), { message: "Schedule days must be a valid number" }).optional(),
  scheduleMinutesPerSession: z.number().int().min(40).max(90).refine((val) => !isNaN(val), { message: "Schedule minutes must be a valid number" }).optional(),
  scheduleWeeks: z.number().int().min(6).max(16).refine((val) => !isNaN(val), { message: "Schedule weeks must be a valid number" }).optional(),
