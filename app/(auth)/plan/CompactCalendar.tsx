...
import type { workouts, workoutLogs as workoutLogsTable } from '@/drizzle/schema';
...
export function CompactCalendar({ workouts, weeks, startDate, logs = [] }: CompactCalendarProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <span>{currentWeek.workouts.length} workouts this week</span>
        <span>
          {currentWeek.days.filter((d) => d.status === 'completed').length} completed
        </span>
      </div>
    </div>
  );
